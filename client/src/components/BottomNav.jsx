...
const BottomNav = ({ activeTab, activeColor = 'var(--color-hunger)' }) => {
  ...
  const getStyle = (tab) => ({
    color: activeTab === tab ? activeColor : 'rgba(0,0,0,0.3)',
    position: 'relative',
  ...
  return (
    <div className="bottom-nav">
      {/* HOME TAB */}
      <button className="nav-tab" onClick={() => handleNav('home')} style={getStyle('home')}>
      ...
      {/* HISTORY TAB */}
      <button className="nav-tab" onClick={() => handleNav('history')} style={getStyle('history')}>
      ...
      {/* SETTINGS TAB */}
      <button className="nav-tab" onClick={() => handleNav('settings')} style={getStyle('settings')}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
